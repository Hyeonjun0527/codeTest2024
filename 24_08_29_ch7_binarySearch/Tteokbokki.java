...
public class Tteokbokki {
    ...
    public static void main(String[] args) throws Exception{
        ...
        int n = inputs[0];
        m = inputs[1];
        tteoks = Arrays.stream(bufferedReader.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        Arrays.sort(tteoks);
        ...
        int result = search(arraysH, 0, maxH + 1);
        System.out.printf("%d  %d  %d\n",sum, h, maxH);
        System.out.println(Arrays.toString(arraysH));
        System.out.println(result);

        //
        int realResult = result >= 0 ? result : -result-1;//sum은 m보다 언제나 커야함. 그러므로 + 1을 해준다.
